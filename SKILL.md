...
```bash
python3 scripts/fetch_norikae_routes.py <出発駅> [経由駅...] <到着駅> [options]
```

Stations are positional: `from [via ...] to` (up to 3 via stations).

Options:
- Time (mutually exclusive): `--departure HH:MM`, `--arrival HH:MM`, `--first`, `--last`, `--unspecified`. If no time flag is given, departs now.
...
- `--no-shinkansen`, `--no-express`, `--no-airline`, `--no-highway-bus`, `--no-local-bus`, `--no-ferry` — by default all transportation types are enabled; pass these flags when the user asks for a specific type
...
| no shinkansen / 不要新干线 | `--no-shinkansen` |
| local trains only / 在来线 | `--no-shinkansen --no-express` |
| no buses / 不要巴士 | `--no-highway-bus --no-local-bus` |
| cash fare / 现金票价 | `--ticket cash` |
| reserved seat / 指定席 | `--seat-preference reserved` |
| Green Car / 绿车 | `--seat-preference green` |
| arrive by / XX点前到 | `--arrival HH:MM` |
| first train / 始発 | `--first` |
| last train / 終電 | `--last` |
...
2. **List lines and directions at the station:**
   ```bash
   python3 scripts/fetch_timetable.py timetable <station-code>
   ```
...
4. **Show a specific train's stop-by-stop schedule (optional):**
   ```bash
   python3 scripts/fetch_timetable.py timetable <station-code> <gid> <train-id>
   ```
